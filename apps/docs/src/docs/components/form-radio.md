...
## Type-Safe Options with TypeScript

When using TypeScript, `BFormRadioGroup` provides full type safety for options arrays with custom field names. The component uses generic types to ensure that your `valueField`, `textField`, and `disabledField` props reference valid properties of your option objects.

### Benefits of Type-Safe Options

- **Compile-time validation**: TypeScript will catch typos in field names before runtime
- **IntelliSense support**: Get autocomplete suggestions for valid field names
- **Refactoring safety**: Renaming properties in your interface will highlight all usage locations
- **Type inference**: The `v-model` automatically infers the correct type based on your `valueField`

### Strongly-Typed Example

The following example demonstrates type-safe radio groups with custom field names. Notice how each interface defines specific property names, and the `valueField`, `textField`, and `disabledField` props reference these properties with full type safety. In the TypeScript code, each option array is strongly typed with interfaces that define custom field names:

<<< DEMO ./demo/RadioTypeSafe.vue

### How It Works

The generic type signature of `BFormRadioGroup` enables this type safety:
...
When you pass options to the component, TypeScript:

1. Infers the `Item` type from your options array
2. Constrains `valueField`, `textField`, and `disabledField` to be valid keys of `Item`
3. Types the `v-model` to match `Item[ValueKey]` (the type of the value field)

This ensures that field name typos are caught at compile time and that your v-model has the correct type throughout your application.
