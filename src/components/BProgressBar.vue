  ...
  setup(props, { slots }) {
    ...
    const computedLabel = computed(() => {
      if (props.showValue || parent?.showValue) {
        return parseFloat(props.value as string).toFixed(
          props.precision as number
        );
      }
    ...
    const width = computed(() => {
      if (props.max || parent?.max) {
        return `${
          ((props.value as number) * 100) /
          parseInt((props.max || parent?.max) as string)
        }%`;
      }
    ...
    return () =>
      h('div', progressProps.value, slots.default?.() || computedLabel.value);
  },
});
</script>
