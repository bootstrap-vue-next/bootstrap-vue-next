...
export const computeLocalChecked = (isChecked: boolean | undefined, propsChecked: unknown, value: string | boolean, uncheckedValue: string | boolean): CheckType => {
    if (isChecked === false) {
...
export const computeIsChecked = (checked: boolean | undefined, propsChecked: CheckType, value: CheckType): boolean | undefined => {
    if (checked)
        return checked

    return propsChecked === true ?
        propsChecked === true :
        propsChecked !== null && value === propsChecked;
}
...
export const handleUpdate = (isChecked: Ref<boolean | undefined>, checked: CheckType, value: string | boolean,
                             uncheckedValue: string | boolean, localChecked: Ref<unknown>, emit: EmitFn): void => {
    ...
    const onInput = (event: Event) => {
        ...
        if (checked === null || checked === undefined) {
            if (isChecked.value)
                ...
                localChecked.value = uncheckedValue
        } else {
            localChecked.value = computeLocalChecked(isChecked.value, checked, value, uncheckedValue);
        }
