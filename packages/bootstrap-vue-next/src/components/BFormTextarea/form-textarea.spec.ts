...
describe('form-textarea', () => {
  ...
  describe('model modifiers', () => {
    ...
    describe('.trim modifier', () => {
      ...
      it('updates textarea element value to trimmed string on blur', async () => {
        const wrapper = mount(BFormTextarea, {
          props: {modelValue: '', modelModifiers: {trim: true}},
        })
        wrapper.element.value = '  hello  '
  ...
  describe('maxRows prop', () => {
    it('sets rows attribute to rows value when maxRows equals rows', () => {
      const wrapper = mount(BFormTextarea, {props: {rows: 4, maxRows: 4}})
      expect(wrapper.attributes('rows')).toBe('4')
    })

    it('falls back to rows 2 when maxRows is greater than rows (auto-resize mode)', () => {
      const wrapper = mount(BFormTextarea, {props: {rows: 4, maxRows: 6}})
      expect(wrapper.attributes('rows')).toBe('2')
    })

    it('updates rows reactively when maxRows is set to match rows', async () => {
      const wrapper = mount(BFormTextarea, {props: {rows: 4, maxRows: 6}})
      expect(wrapper.attributes('rows')).toBe('2')
      await wrapper.setProps({maxRows: 4})
      expect(wrapper.attributes('rows')).toBe('4')
    })
  })

  describe('noAutoShrink prop', () => {
    it('applies resize none style when noAutoShrink is true', () => {
      const wrapper = mount(BFormTextarea, {props: {noAutoShrink: true}})
      expect(wrapper.element.style.resize).toBe('none')
    })

    it('does not apply resize none style from noAutoShrink when false and noResize is false', () => {
      const wrapper = mount(BFormTextarea, {props: {noAutoShrink: false, noResize: false}})
      expect(wrapper.element.style.resize).not.toBe('none')
    })

    it('updates noAutoShrink reactively', async () => {
      const wrapper = mount(BFormTextarea, {props: {noAutoShrink: false, noResize: false}})
      expect(wrapper.element.style.resize).not.toBe('none')
      await wrapper.setProps({noAutoShrink: true})
      expect(wrapper.element.style.resize).toBe('none')
    })
  })

  describe('autofocus', () => {
    it('focuses the textarea element on mount when autofocus is true', async () => {
      const wrapper = mount(BFormTextarea, {
    ...
    it('does not focus textarea on mount when autofocus is false', async () => {
      const wrapper = mount(BFormTextarea, {
  ...
  describe('null modelValue', () => {
    it('renders empty value when modelValue is null', () => {
      const wrapper = mount(BFormTextarea, {props: {modelValue: null}})
      expect(wrapper.element.value).toBe('')
    })
  })
})
